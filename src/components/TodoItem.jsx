    ...
    >
      <button
        ...
        className={`shrink w-6 h-6 rounded-lg flex items-center justify-center transition-all duration-300 transform hover:scale-110 active: scale-95 ${
          todo.completed
            ? "bg-linear-to-br from-emerald-400 to-teal-400 border-emerald-400 shadow-lg shadow-emerald-400/50"
            : "border-white/30 hover:border-violet-400 hover:bg-violet-500/20 "
        }`}
      >
        {todo.completed && (
          <FaCheckSquare size={14} className="text-white font-bold" />
        )}
      </button>
      ...
      {isEditing ? (
        <>
          <input
            ...
            className="flex-1 px-3 py-1.5 bg-white/10 placeholder-white/40 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 font-medium text-sm border-white/10"
            autoFocus
          />
        </>
      ) : (
